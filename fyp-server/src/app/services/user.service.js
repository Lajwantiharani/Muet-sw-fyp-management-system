...
const retrieveAll = async (roleOrStatus, authUserId, { query = {}, current = 1, size = 10, sort = {} } = {}) => tryCatch(async () => {
    // constuct the query to search on fields
    const searchQuery = constructQuery(query, true);

    // validate role or status
    const finalQuery = { $and: [...[roleOrStatus != "all" ? { $or: [{ status: roleOrStatus }, { role: roleOrStatus }] } : {}], searchQuery, { _id: { $ne: authUserId } }] };

    // retrieve documents with pagination
    return await pagination(User, { query: finalQuery, current, size, sort });
});
...
// method to retrieve all admins' ids
const adminIds = async (id) => {
    return await tryCatch(async () => {
        const adminIds = await User.find({ role: "admin" }).select("_id").lean();
        return adminIds.map(admin => admin._id.toString());
    });
};

export default { retrieveAll, retrieveOne, create, update, delete: del, adminIds };