...
const retrieveAll = async (status, { query = {}, current = 1, size = 10, sort = {} } = {}) => tryCatch(async () => {
    // constuct the query to search on fields
    const searchQuery = constructQuery(query, true);

    // retrieve status based projects
    const finalQuery = { $and: [(status != "all" ? { status } : {}), searchQuery] };
...
const retrieveMany = async ({ supervisorId, status }, { query = {}, current = 1, size = 10, sort = {} } = {}) => tryCatch(async () => {
    // constuct the query to search on fields
    const searchQuery = constructQuery(query, true);

    // construct the query to match supervisor projects and status if provided
    const queries = [
        { supervisor: createMongooseObjectId(supervisorId) },
        ...(status != "all" ? [{ status }] : [])
    ];

    // merge queries to retrieve specified documents
    const finalQuery = { $and: [...queries, searchQuery] };
...
const retrieveOne = async (queryId, direct = false) => {
    if (direct) {
        // return retrieved project document or null
        return await tryCatch(() => Project.findOne(queryId)
            .populate(populateOptions.project)
        );
    }
