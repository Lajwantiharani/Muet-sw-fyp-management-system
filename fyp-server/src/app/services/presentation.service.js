...
const retrieveAll = async ({ requestUser, status }, { query = {}, current = 1, size = 10, sort = {} } = {}) => tryCatch(async () => {
    ...
    const searchQuery = constructQuery({ ...query, ...(status != "all" ? { status } : {}) }, true);
...
const retrieveMany = async (refId, { query = {}, current = 1, size = 10, sort = {} } = {}) => tryCatch(async () => {
    ...
    const project = await projectService.retrieveOne(refId);

    // constructor final query
    const finalQuery = { $and: [{ project: project?._id ?? null }, searchQuery] };
...
const retrieveOne = async (queryId, extraQuery = null) => {
    ...
    if (typeof queryId === "object") {
        ...
        // convert ID string to mongoose ObjectId
        query = { _id: createMongooseObjectId(typeof queryId == "string" ? queryId : query.toString()) };
    }
