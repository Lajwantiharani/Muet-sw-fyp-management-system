...
proposalRoutes.post('/retrieve/:status',
...
// many proposal documents against id
proposalRoutes.post('/:proposalId',
    auth.authenticate, // middleware to authenticate request user based on JWT token
    auth.authorize("student"),  // middleware to authorize role(s) to access the route
    proposalController.retrieveMany // controller method to retrieve specified proposal docuement
);
