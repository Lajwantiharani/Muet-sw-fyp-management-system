...
const send = async (email, options) => {
    ...
    if (options.template) { // send html template from views directory as email body
        mailOptions.html = await getHtmlTempalate(options.template, {
            ...options,
            baseUrl: env.app.origin,
            clientBaseUrl: env.client.origin,
            supportEmail: env.support.email,
            supportPhone: env.support.phone,
        });
